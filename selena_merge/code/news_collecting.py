        ...
        while True:
          ...
          for item in news_list :
            if len(item.select("div.info_group a")) == 2 :
              news_data = get_news(session, item.select("div.info_group a")[1]['href'])
              if news_data:
...
#data 폴더에 csv로 파일 저장 #백슬래시 아닌 기본 슬래시 사용 :  이스케이프 시퀀스로 해석되는 것을 방지
rows.to_csv('C:/Users/selena/songmac/Project-Tesla-StockSwing-Analysis/selena_merge/data/original_data/news_data/2023_Dec_news_data.csv', encoding='utf-8-sig')
